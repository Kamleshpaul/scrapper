...
program
  ...
  .arguments('<url>')
  .action(async (url) => {
    const username = await scrapeInstagramUsername(url, { headless: 'new' }); 
...
async function scrapeInstagramUsername(url, options) {
  try {
    ...
    const username = await page.evaluate(() => {
      const followButton = Array.from(document.querySelectorAll('button'))
        .find(button => button.textContent.includes('Follow'));

      if (followButton) {
        const aTag = followButton.parentElement.parentElement.querySelector('a');
        return aTag ? aTag.getAttribute('href').replace('/', '').replace('/', '') : null;
      }
      return null;
    });
